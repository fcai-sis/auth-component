...
import { TeachingAssistantModel, UserModel } from "@fcai-sis/shared-models";

const loginTaHandler = async (req: Request, res: Response) => {
  try {
    const { email, password } = req.body;

    const instructor = await TeachingAssistantModel.findOne({ email });
